...
const marcasItems = [
  { label: "ABS Company", href: "/marcas/abs-company" },
];
                  ...
                  {marcasItems.map((item, i) => (
                    <Link
                      ...
                      className={`
                        ...
                        text-white/50 hover:text-white hover:bg-white/5
                        transition-colors duration-150
                        ${i < marcasItems.length - 1 ? "border-b border-white/5" : ""}
                      `}
                    >
                      <span className="w-1 h-1 rounded-full bg-[#E8420C] shrink-0" />
                      {item.label}
      ...
      {menuOpen && (
        <div className="lg:hidden bg-black border-t border-white/10">
          <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col">
            ...
            <div className="border-b border-white/[0.06]">
              <button
                onClick={() => setMobileMarcasOpen(!mobileMarcasOpen)}
                className="flex items-center justify-between w-full text-white/60 text-[16px] font-semibold tracking-[0.08em] uppercase py-3.5 hover:text-white transition-colors"
              >
                Marcas
                <ChevronDown
                  size={14}
                  className={`transition-transform duration-200 ${mobileMarcasOpen ? "rotate-180 text-[#E8420C]" : ""}`}
                />
              </button>

              {mobileMarcasOpen && (
                <div className="flex flex-col pl-4 border-l-2 border-[#E8420C]/40 mb-3">
                  {marcasItems.map((item) => (
                    <Link
                      key={item.label}
                      href={item.href}
                      onClick={() => { setMenuOpen(false); setMobileMarcasOpen(false); }}
                      className="text-white/40 text-[16px] font-medium tracking-wide uppercase py-2.5 hover:text-white transition-colors"
                    >
                      {item.label}
                    </Link>
